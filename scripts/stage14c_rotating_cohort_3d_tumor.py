"""
Stage 14C: Rotating cohort 3D tumor-model animation (GIF).

Animated companion to the Stage 14B gallery: builds the real-mask-derived 3D tumor
models for a few cohort patients and rotates them together as a single GIF.

Same method/wording as Stage 13/14A/14B: spatial tumor-node graphs from recovered
...
def ensure_usable_3d(mask3d):
    return clean_mask3d(crop_bbox_3d(mask3d, pad=2))
...
def make_mesh(mask3d, max_faces=6000):
...
def tumor_graph(mask3d, transform, max_nodes=35, knn=4):
...
def draw_panel(ax, mask3d, title):
    coords, faces, transform = make_mesh(mask3d)
    mesh = Poly3DCollection(coords[faces], alpha=0.28)
    ...
    ax.scatter(centers[:, 0], centers[:, 1], centers[:, 2], c=cols, s=16, depthshade=True)
...
def main():
    ...
    pats = pats[:args.n]
    print(f"[info] rotating GIF for: {pats}")
    ...
    ncol = 2 if len(masks) > 1 else 1
    nrow = int(np.ceil(len(masks) / ncol))
    fig = plt.figure(figsize=(4.6 * ncol, 4.4 * nrow))
    axes = []
    for i, (pid, m) in enumerate(masks, 1):
        ax = fig.add_subplot(nrow, ncol, i, projection="3d")
        draw_panel(ax, m, pid)
        axes.append(ax)
    fig.suptitle("Stage 14C: rotating real-mask-derived 3D tumor models (ISPY2 cohort)", fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.95])

    def update(frame):
        az = 360.0 * frame / args.frames
        for ax in axes:
            ax.view_init(elev=18, azim=az)
        return []

    anim = FuncAnimation(fig, update, frames=args.frames, interval=1000 / args.fps, blit=False)
    out = FIGS / "720_stage14c_rotating_cohort_3d_tumor.gif"
    anim.save(out, writer=PillowWriter(fps=args.fps))
    plt.close(fig)
    size_mb = out.stat().st_size / 1e6
    print(f"[done] wrote {out}  ({size_mb:.1f} MB)")
    if size_mb > 45:
        print("[warn] GIF is large; rerun with fewer --frames or fewer --n if git push complains.")

    rep = REPORTS / "Stage14C_Rotating_Cohort_3D_Tumor_Report.md"
    rep.write_text(
        "# Stage 14C Rotating Cohort 3D Tumor Animation\n\n"
        f"Rotating GIF of real-mask-derived 3D tumor models for {len(masks)} cohort patients.\n"
        "Blue = core nodes, orange = periphery. Edges are spatial links, not vessels or measured flow.\n\n"
        "## Patients shown\n" + "\n".join(f"- {pid} (voxels={int(m.sum())})" for pid, m in masks) + "\n\n"
        f"## Settings\n- Frames: {args.frames}\n- FPS: {args.fps}\n\n"
        "## Output\n- reports/figures/720_stage14c_rotating_cohort_3d_tumor.gif\n\n"
